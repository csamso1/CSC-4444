...
loc_A, loc_B, loc_C = (0, 0), (1, 0), (2, 0) # The three locations for my Vacuum world
...
def TableDrivenVacuumAgent():
    ...
    table = {((loc_A, 'Clean'),): 'Right',
             ((loc_A, 'Dirty'),): 'Suck',
             ((loc_B, 'Clean'),): random.choice(['Right', 'Left']),
             ((loc_B, 'Dirty'),): 'Suck',
             ((loc_C, 'Clean'),): 'Left',
             ((loc_C, 'Dirty'),): 'Suck',
...
def HellenKellerAgent():
    ...
    return Agent(program)
#class SuperModelBasedVacuumAgent(Agent):
    
 #   def seeTheWorld(self, theWorld):
  #      model[loc_A] = theWorld[0]
   #     model[loc_B] = theWorld[1]
    #    model[loc_C] = theWorld[2]
# ______________________________________________________________________________
...
class TrivialVacuumEnvironment(Environment):
    ...
    def execute_action(self, agent, action):
        ...
        if self.status[loc_A] == 'Dirty':
            agent.performance -= 2
        if self.status[loc_B] == 'Dirty':
            agent.performance -= 2
        if self.status[loc_C] == 'Dirty':
            agent.performance -= 2
